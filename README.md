...
- 按住 `LT`：左臂位置控制；松开停止。
- 按住 `LG`：左臂姿态控制；松开停止。
- 同时按住 `LT+LG`：左臂位置和姿态同时控制。
- 按住 `RT`：右臂位置控制；松开停止。
- 按住 `RG`：右臂姿态控制；松开停止。
- 同时按住 `RT+RG`：右臂位置和姿态同时控制。

按下 LT/LG/RT/RG 时会捕获当前位置和水平朝向锚点。在单一模式和组合模式之间切换时，控制器会慢停并重新捕获锚点，避免未控制的自由度跳回旧锚点；松开一个键后会继续保持另一个键对应的单一控制模式。操作者改变站立朝向后，应松开并重新按下对应按键。面对、背对或与机器人呈 `+/-90` 度时，前后/左右映射保持一致。
